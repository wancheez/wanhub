...
- `GET /telemt` — телеметрия telemt-прокси (Prometheus `/metrics` парсится в курированный дашборд, обновление раз в 5с). URL задаётся в `TELEMT_METRICS_URL`.
- `GET /api/telemt` — тот же снимок в JSON
...
Триггер: в группах сообщение должно начинаться со слова **«Чат»** (case-insensitive). В личке префикс не нужен — бот отвечает на любое сообщение. Команды-слэши работают всегда.
...
- **`/device`** — состояние Pi
- **`/telemt`** — телеметрия telemt-прокси (требует `TELEMT_METRICS_URL`)
...
Whitelist чатов хранится в SQLite (`logs/chat.sqlite3`, таблица `chat_whitelist`) и наполняется в рантайме:

1. В `.env` указываешь `TELEGRAM_ADMIN_ID` — это ты. Админ всегда считается одобренным (bootstrap, в БД может не быть).
2. Когда новый юзер пишет боту в личку — бот отвечает «📨 Запрос отправлен админу» и шлёт тебе сообщение с кнопками `[✅ Одобрить] [❌ Отклонить]`.
3. Аналогично, когда бота добавили в новую группу — событие `my_chat_member` триггерит запрос.
4. После `✅` чат получает «✅ Доступ открыт» и работает дальше; после `❌` — молчание навсегда.
5. Pending-чаты повторных уведомлений не получают (никаких спам-волн админу). Снять `denied` можно только руками в БД.
...
Сервер слушает `http://0.0.0.0:8000/`. Для запуска бота нужно в `.env` положить `TELEGRAM_BOT_TOKEN` от [@BotFather](https://t.me/BotFather) и `TELEGRAM_ADMIN_ID` — свой Telegram user_id (`@userinfobot` подскажет). После старта напиши боту — ты как админ пройдёшь сразу. Все остальные чаты будут проходить через approve-флоу.
