...
class DealSession:
    ...
    # Защёлка финализации: хендлерный `_finalize_and_summarize` обязан отработать
    # ровно один раз на партию. Без неё гонка (повторный /deal по зависшей
    # FINISHED-сессии, клик + авто-таймер) приводила к двойной записи в БД и
    # двойному summary. Ставится синхронно до первого await финализации.
