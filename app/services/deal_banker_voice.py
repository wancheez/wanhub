...
def _fmt_short(v: int) -> str:
    """Короткий формат суммы: 3_000_000 → «3М», 1_300_000 → «1.3М», 500_000 → «500к».

    Остаток не отбрасываем: офер банкира выше миллиона кратен 100к (см.
    `_round_clean` в `app.services.deal`), поэтому хвостовой знак после запятой
    точно передаёт сумму. Иначе 1 300 000 ₽ озвучивалось как «1М».
    """
    if v >= 1_000_000:
        return _trim(v / 1_000_000) + "М"
    if v >= 1_000:
        return _trim(v / 1_000) + "к"
    return str(v)
